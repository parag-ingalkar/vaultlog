...
export default function VaultGrantsPage() {
  ...
  const { canManageGrants } = usePermissions();
  const grants = useVaultGrantsQuery(vaultId, canManageGrants);
  const members = useMembersQuery(canManageGrants);
  ...
  return (
    <CapabilityGuard allowed={canManageGrants}>
      <div className="space-y-6">
        ...
        <Card>
          ...
          <form onSubmit={handleUpsert} className="mt-4 space-y-3">
            ...
            <MutationError error={upsertGrant.error} />
          </form>
        </Card>
